...
pub struct ToastConfig {
    ...
    pub osade: OsadeToastConfig,
    pub clipboard: ClipboardToastConfig,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize)]
#[serde(default)]
pub struct OsadeToastConfig {
    pub position: ToastOsadePosition,
}
...
pub struct SessionConfig {
    ...
    /// when restoring a Osade session. Default: true.
...
pub struct KeysConfig {
    ...
    /// Detach the current client from its Osade server. Default: "prefix+q".
    ...
    /// Local-client shortcut that sends a clipboard image to a remote Osade session. Default: "ctrl+v".
...
pub struct WorktreesConfig {
    /// Root directory under which Osade creates <repo>/<branch-slug> checkouts.
...
pub struct UiConfig {
    ...
    /// Terminal width at or below which Osade uses the mobile single-column layout. Default: 64.
    pub mobile_width_threshold: u16,
    /// Capture mouse input for Osade's mouse UI. Default: true.
    ...
    /// Retired setting that Osade wrote before the workspace filter was removed.
...
pub struct RemoteConfig {
    /// Add keepalive fallbacks and private connection reuse for `osade --remote`.
...
pub struct ExperimentalConfig {
    /// Allow launching osade inside an existing osade pane. Default: false.
...
impl Default for WorktreesConfig {
    fn default() -> Self {
        Self {
            directory: "~/.osade/worktrees".into(),
        }
    }
}
...
impl Default for ToastConfig {
    fn default() -> Self {
        Self {
            ...
            delay_seconds: 1,
            osade: OsadeToastConfig::default(),
            clipboard: ClipboardToastConfig::default(),
        }
    }
}

impl Default for OsadeToastConfig {
    fn default() -> Self {
        Self {
            position: ToastOsadePosition::BottomRight,
        }
    }
}
...
"#;
    ...
    fn worktrees_directory_defaults_and_parses() {
        let default_config = Config::default();
        assert_eq!(default_config.worktrees.directory, "~/.osade/worktrees");

        let toml = r#"
[worktrees]
directory = "~/Projects/osade-worktrees"
"#;
        let config: Config = toml::from_str(toml).unwrap();
        assert_eq!(config.worktrees.directory, "~/Projects/osade-worktrees");
    }
...
"#;
        ...
        assert_eq!(config.ui.toast.osade.position, ToastOsadePosition::TopLeft);
    ...
    fn toast_config_defaults_preserve_existing_behavior_with_delay() {
        ...
        assert_eq!(
            config.ui.toast.osade.position,
            ToastOsadePosition::BottomRight
        );
        assert!(config.ui.toast.clipboard.enabled);
        assert_eq!(
            config.ui.toast.clipboard.position,
...
"#;
    ...
    #[test]
    fn toast_config_legacy_enabled_true_maps_to_osade() {
        let toml = r#"
[ui.toast]
enabled = true
"#;
        let config: Config = toml::from_str(toml).unwrap();
        assert_eq!(config.ui.toast.delivery, ToastDelivery::Osade);
    }
