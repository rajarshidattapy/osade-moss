...
pub(crate) const OSADE_PANE_ID_ENV_VAR: &str = "OSADE_PANE_ID";
pub(crate) const OSADE_TAB_ID_ENV_VAR: &str = "OSADE_TAB_ID";
pub(crate) const OSADE_WORKSPACE_ID_ENV_VAR: &str = "OSADE_WORKSPACE_ID";
...
pub(crate) fn apply_pane_base_env(cmd: &mut CommandBuilder) {
    ...
    if let Ok(executable) = std::env::current_exe() {
        cmd.env("OSADE_BIN_PATH", executable);
    }
}
...
pub(crate) fn grok_dir() -> io::Result<PathBuf> {
    // GROK_CONFIG_DIR is a osade-level override only (primarily a test
