...
const originalEnvironment = {
  OSADE_ENV: process.env.OSADE_ENV,
  OSADE_OMP_IDLE_DEBOUNCE_MS: process.env.OSADE_OMP_IDLE_DEBOUNCE_MS,
  OSADE_PANE_ID: process.env.OSADE_PANE_ID,
  OSADE_SOCKET_PATH: process.env.OSADE_SOCKET_PATH,
};
...
const integrations = [
  { name: "Pi", modulePath: "./pi/osade-agent-state.ts" },
  { name: "Oh My Pi", modulePath: "./omp/osade-agent-state.ts" },
] as const;

const socketPlugins = [
  {
    name: "OpenCode",
    modulePath: "./opencode/osade-agent-state.js",
    sessionID: "opencode-session",
  },
  { name: "Kilo", modulePath: "./kilo/osade-agent-state.js", sessionID: "kilo-session" },
] as const;
...
function configureIntegrationEnvironment(recordingSocketPath: string) {
  process.env.OSADE_ENV = "1";
  process.env.OSADE_SOCKET_PATH = recordingSocketPath;
  process.env.OSADE_PANE_ID = "test:p1";
}
...
async function startRecordingServer(name: string): Promise<unknown[]> {
  const recordingSocketPath = join(tmpdir(), `osade-${name}-${process.pid}.sock`);
...
for (const socketPlugin of socketPlugins) {
  test(`${socketPlugin.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `osade-${socketPlugin.name.toLowerCase()}-${process.pid}.sock`;
    configureIntegrationEnvironment(markerPath);
    Object.defineProperty(process, "platform", { value: "win32" });
    const connectedEndpoint = captureConnectionEndpoint();

    const { OsadeAgentStatePlugin } = await importFresh(socketPlugin.modulePath);
    const plugin = await OsadeAgentStatePlugin();
    await plugin.event({
      event: {
...
test("OpenCode stays disabled without the Osade socket environment", async () => {
  process.env.OSADE_ENV = "1";
  process.env.OSADE_PANE_ID = "test:p1";
  delete process.env.OSADE_SOCKET_PATH;

  const { OsadeAgentStatePlugin } = await importFresh("./opencode/osade-agent-state.js");

  expect(await OsadeAgentStatePlugin()).toEqual({});
});

for (const integration of integrations) {
  test(`${integration.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `osade-${integration.name.toLowerCase().replaceAll(" ", "-")}-${process.pid}.sock`;
    configureIntegrationEnvironment(markerPath);
    Object.defineProperty(process, "platform", { value: "win32" });
    const connectedEndpoint = captureConnectionEndpoint();
...
test("OMP accepts POSIX and Windows session paths", async () => {
  const { isAbsoluteSessionPath } = await importFresh("./omp/osade-agent-state.ts");
...
test("Pi reports idle only after the agent settles", async () => {
  ...
  const { handlers, pi } = createExtensionHarness();
  const { default: install } = await importFresh("./pi/osade-agent-state.ts");
  install(pi);
...
test("Pi settlement preserves explicit blocked-state precedence", async () => {
  ...
  handlers.get("agent_start")?.({}, context);
  await waitFor(() => requestStates(requests).length === 2);
  eventHandlers.get("osade:blocked")?.({ active: true, label: "approval" }, context);
  await waitFor(() => requestStates(requests).length === 3);
  ...
  eventHandlers.get("osade:blocked")?.({ active: false }, context);
...
test("Pi waits for a replacement session report before publishing state", async () => {
  const recordingSocketPath = join(tmpdir(), `osade-pi-session-order-${process.pid}.sock`);
  socketPath = recordingSocketPath;
  await rm(recordingSocketPath, { force: true });
...
test("Oh My Pi retries working before a queued idle state", async () => {
  ...
  process.env.OSADE_OMP_IDLE_DEBOUNCE_MS = "0";
  const { handlers, pi } = createExtensionHarness();

  const { default: install } = await importFresh("./omp/osade-agent-state.ts");
  install(pi);
