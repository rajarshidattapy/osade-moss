...
const OSADE_ENV = process.env.OSADE_ENV;
const socketPath = process.env.OSADE_SOCKET_PATH;
const socketEndpoint =
  process.platform === "win32" && socketPath ? `\\\\.\\pipe\\${socketPath}` : socketPath;
const paneId = process.env.OSADE_PANE_ID;
const source = "osade:omp";

function enabled() {
  return OSADE_ENV === "1" && !!socketPath && !!paneId;
}
...
const idleDebounceMs = parseDurationEnv("OSADE_OMP_IDLE_DEBOUNCE_MS", 250);
const retryGraceMs = parseDurationEnv("OSADE_OMP_RETRY_GRACE_MS", 2500);
...
export default function (pi) {
  ...
  pi.events.on("osade:blocked", (data) => {
    if (!rootSession) {
      return;
    }
    if (!data?.active) {
