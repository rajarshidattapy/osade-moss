...
# OSADE_INTEGRATION_ID=devin
# OSADE_INTEGRATION_VERSION=2

set -eu

action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/osade-devin-hook.XXXXXX")" || exit 0
...
OSADE_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
...
def load_session_list(project_dir: str | None):
    injected = os.environ.get("OSADE_DEVIN_LIST_JSON")
...
hook_input = load_hook_input(os.environ.get("OSADE_HOOK_INPUT_FILE"))
