...
fn windows_powershell_encoded_hook_command_preserves_script_invocation() {
    ...
    let hook_path = Path::new(r"C:\Users\O'Neil λ\App Data\hooks\osade-agent-state.ps1");
    ...
    assert_eq!(
        ...
        r"& 'C:\Users\O''Neil λ\App Data\hooks\osade-agent-state.ps1' session"
    );
}
...
fn windows_antigravity_cli_hook_command_uses_encoded_powershell() {
    let hook_path = Path::new(r"C:\Users\reporter\.gemini\config\hooks\osade-agent-state.ps1");
    assert_eq!(
...
fn enforce_agent_version_warns_when_binary_missing() {
    let requirement = AgentVersionRequirement {
        label: "kimi code",
        binary: "osade-test-binary-that-does-not-exist",
        args: &["--version"],
        min_version: "0.14.0",
    };
...
fn unique_base() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "osade-integration-install-test-{}-{}",
        std::process::id(),
...
fn integration_recommendation_installs_available_or_outdated_targets() {
    let mut recommendation = IntegrationRecommendation {
        ...
        command: "claude",
        available: false,
        path: PathBuf::from("/tmp/osade-agent-state.sh"),
...
#[test]
fn install_omp_preserves_non_osade_file_with_pi_install_name() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let ext_dir = home.join(".omp/agent/extensions");
...
fn outdated_integrations_treat_missing_version_marker_as_legacy() {
    ...
    fs::write(&extension_path, "// installed by osade\n").unwrap();
    std::env::set_var("HOME", &home);
...
fn outdated_integrations_detect_previous_pi_version() {
    ...
    fs::write(
        &extension_path,
        "// OSADE_INTEGRATION_ID=pi\n// OSADE_INTEGRATION_VERSION=4\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
#[test]
fn uninstall_claude_removes_osade_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let claude_dir = home.join(".claude");
...
#[test]
fn uninstall_codex_removes_osade_hooks_and_leaves_config_alone() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let codex_dir = home.join(".codex");
    ...
    for event in COPILOT_REMOVED_LIFECYCLE_HOOK_EVENTS {
        if let Some(entries) = settings["hooks"].get(event) {
            assert!(
                ...
                "expected osade hooks.{event} entries to be removed"
            );
        }
    }
    assert!(settings["hooks"].get("sessionStart").is_none());
...
#[test]
fn uninstall_copilot_removes_osade_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let copilot_dir = home.join(".copilot");
...
#[test]
fn uninstall_devin_removes_osade_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let xdg_config = base.join("xdg");
    let devin_dir = xdg_config.join("devin");
...
#[test]
fn uninstall_droid_removes_osade_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let droid_dir = home.join(".factory");
...
fn install_hermes_is_idempotent_for_enabled_entry() {
    ...
    assert_eq!(config.matches("osade-agent-state").count(), 1);
...
fn install_hermes_is_idempotent_for_quoted_flat_plugin_entry() {
    ...
    fs::write(
        hermes_dir.join("config.yaml"),
        "plugins:\n  - \"osade-agent-state\" # installed by osade\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
fn uninstall_hermes_removes_plugin_and_enabled_entry() {
    ...
    assert!(!config.contains("osade-agent-state"));
...
fn uninstall_hermes_removes_flow_plugin_list_entry() {
    ...
    fs::write(
        hermes_dir.join("config.yaml"),
        "plugins: [other-plugin, osade-agent-state]\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
fn bundled_integration_assets_report_session_refs() {
    ...
    assert!(
        CODEX_HOOK_ASSET.contains("OSADE_HOOK_INPUT_FILE")
    ...
    assert!(KIMI_HOOK_ASSET.contains("source\": \"osade:kimi"));
    ...
    assert!(DEVIN_HOOK_ASSET.contains("OSADE_DEVIN_LIST_JSON"));
    ...
    assert!(KILO_PLUGIN_ASSET.contains("SOURCE = \"osade:kilo\""));
    ...
    assert!(QODERCLI_HOOK_ASSET.contains("OSADE_PANE_ID"));
    ...
    assert!(CURSOR_HOOK_ASSET.contains("OSADE_INTEGRATION_ID=cursor"));
    ...
    assert!(MASTRACODE_HOOK_ASSET.contains("OSADE_INTEGRATION_ID=mastracode"));
    assert!(MASTRACODE_HOOK_ASSET.contains("OSADE_INTEGRATION_VERSION=2"));
    ...
    assert!(GROK_HOOK_ASSET.contains("OSADE_INTEGRATION_ID=grok"));
    ...
    assert!(GROK_HOOK_ASSET.contains("osade:grok"));
...
#[test]
fn uninstall_qodercli_removes_osade_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let qoder_dir = base.join(".qoder");
    fs::create_dir_all(&qoder_dir).unwrap();
...
fn install_qwen_writes_session_hook_and_preserves_settings() {
    ...
    assert!(hook_asset.contains("OSADE_INTEGRATION_ID=qwen"));
    assert!(hook_asset.contains("OSADE_INTEGRATION_VERSION=1"));
    assert!(hook_asset.contains("osade:qwen"));
...
#[test]
fn uninstall_qwen_removes_only_osade_hook() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let qwen_dir = base.join(".qwen");
    fs::create_dir_all(&qwen_dir).unwrap();
...
#[test]
fn uninstall_cursor_removes_osade_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let cursor_dir = base.join(".cursor");
    fs::create_dir_all(&cursor_dir).unwrap();
...
fn install_mastracode_writes_hook_and_updates_hooks_json() {
    ...
    for (event, action) in MASTRACODE_HOOK_EVENTS {
        let entries = hooks.get(event).and_then(Value::as_array).unwrap();
        assert_eq!(entries.len(), 1, "{event} should have one Osade hook");
...
fn install_grok_writes_hook_and_config() {
    ...
    #[cfg(not(windows))]
    {
        ...
        assert!(command.contains("osade-agent-state.sh"));
        assert!(command.ends_with(" session"));
    }
...
#[test]
fn uninstall_mastracode_removes_osade_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let original_home = std::env::var_os("HOME");
    std::env::set_var("HOME", &base);
...
fn install_antigravity_cli_writes_hook_and_updates_hooks_json() {
    ...
    // Osade entries live under a named hook block; Antigravity CLI rejects a
    ...
    for (event, action) in ANTIGRAVITY_CLI_HOOK_EVENTS {
        ...
        assert_eq!(entries.len(), 1, "{event} should hold one Osade entry");
    ...
    // turn rather than process exit, so Osade never claims lifecycle authority
...
fn antigravity_cli_v2_install_is_outdated_until_reinstalled() {
    ...
    fs::write(
        ...
        ANTIGRAVITY_CLI_HOOK_ASSET
            .replace("OSADE_INTEGRATION_VERSION=3", "OSADE_INTEGRATION_VERSION=2"),
    )
    .unwrap();
    std::env::set_var(ANTIGRAVITY_CLI_CONFIG_DIR_ENV_VAR, &agy_dir);
...
#[test]
fn install_antigravity_cli_rewrites_stale_osade_block() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let agy_dir = base.join(".gemini").join("config");
    fs::create_dir_all(&agy_dir).unwrap();
    // An older Osade install claimed lifecycle authority, wrapped events in
    // matcher/hooks, and left entries Antigravity CLI now rejects.
    fs::write(
        agy_dir.join("hooks.json"),
        r#"{"osade":{"Stop":[{"matcher":"*","hooks":[{"type":"command","command":"stale"}]}],"PostInvocation":[{"type":"command","command":"stale idle"}],"Legacy":[]}}"#,
    )
    .unwrap();
    std::env::set_var(ANTIGRAVITY_CLI_CONFIG_DIR_ENV_VAR, &agy_dir);
    ...
    // The block is Osade-owned and rewritten wholesale, so a stale lifecycle
    ...
    assert!(entries[0]
        .get("command")
        .and_then(Value::as_str)
        .is_some_and(|command| command.contains("osade-agent-state")));
...
fn grok_v1_integration_status_is_current() {
    ...
    fs::create_dir_all(&grok_dir).unwrap();
    std::env::set_var(GROK_CONFIG_DIR_ENV_VAR, &grok_dir);
    // A real install writes both the hook script and hooks/osade.json.
    install_grok().unwrap();
...
fn grok_status_reports_outdated_when_hook_config_missing_or_broken() {
    ...
    fs::write(
        &config_path,
        r#"{"hooks":{"SessionStart":[{"hooks":[{"type":"command","command":"echo osade-agent-state.sh"}]}]}}"#,
    )
    .unwrap();
    assert_eq!(grok_state(), IntegrationStatusKind::Outdated);
...
fn uninstall_antigravity_cli_removes_hooks_json_entries_and_hook_file() {
    ...
    // The Osade block is gone and unrelated named hooks survive.
...
fn grok_dir_honors_grok_home_after_config_dir_seam() {
    ...
    // The osade-level test seam still wins over GROK_HOME when set.
