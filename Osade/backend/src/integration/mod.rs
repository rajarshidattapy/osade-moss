...
pub(crate) use env::{
    apply_pane_base_env, OSADE_PANE_ID_ENV_VAR, OSADE_TAB_ID_ENV_VAR, OSADE_WORKSPACE_ID_ENV_VAR,
};
...
const PI_EXTENSION_INSTALL_NAME: &str = "osade-agent-state.ts";
const PI_EXTENSION_ASSET: &str = include_str!("assets/pi/osade-agent-state.ts");
const PI_INTEGRATION_VERSION: u32 = 8;
const OMP_EXTENSION_INSTALL_NAME: &str = "osade-omp-agent-state.ts";
const OMP_EXTENSION_ASSET: &str = include_str!("assets/omp/osade-agent-state.ts");
const OMP_INTEGRATION_VERSION: u32 = 9;
const CLAUDE_HOOK_INSTALL_NAME: &str = if cfg!(windows) {
    "osade-agent-state.ps1"
} else {
    "osade-agent-state.sh"
};
const CLAUDE_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/claude/osade-agent-state.ps1")
} else {
    include_str!("assets/claude/osade-agent-state.sh")
};
...
const CODEX_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/codex/osade-agent-state.ps1")
} else {
    include_str!("assets/codex/osade-agent-state.sh")
};
...
const KIMI_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/kimi/osade-agent-state.ps1")
} else {
    include_str!("assets/kimi/osade-agent-state.sh")
};
const KIMI_INTEGRATION_VERSION: u32 = 7;
const KIMI_CONFIG_BLOCK_BEGIN: &str = "# >>> osade kimi integration";
const KIMI_CONFIG_BLOCK_END: &str = "# <<< osade kimi integration";
...
const COPILOT_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/copilot/osade-agent-state.ps1")
} else {
    include_str!("assets/copilot/osade-agent-state.sh")
};
...
const DEVIN_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/devin/osade-agent-state.ps1")
} else {
    include_str!("assets/devin/osade-agent-state.sh")
};
...
const DROID_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/droid/osade-agent-state.ps1")
} else {
    include_str!("assets/droid/osade-agent-state.sh")
};
...
const OPENCODE_PLUGIN_INSTALL_NAME: &str = "osade-agent-state.js";
const OPENCODE_PLUGIN_ASSET: &str = include_str!("assets/opencode/osade-agent-state.js");
const OPENCODE_TUI_PLUGIN_INSTALL_NAME: &str = "osade-tui-session.js";
const OPENCODE_TUI_PLUGIN_SPEC: &str = "./osade-tui-session.js";
const OPENCODE_TUI_PLUGIN_ASSET: &str = include_str!("assets/opencode/osade-tui-session.js");
const OPENCODE_INTEGRATION_VERSION: u32 = 10;
const KILO_PLUGIN_INSTALL_NAME: &str = "osade-agent-state.js";
const KILO_PLUGIN_ASSET: &str = include_str!("assets/kilo/osade-agent-state.js");
const KILO_INTEGRATION_VERSION: u32 = 4;
const HERMES_PLUGIN_INSTALL_NAME: &str = "osade-agent-state";
...
const QODERCLI_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/qodercli/osade-agent-state.ps1")
} else {
    include_str!("assets/qodercli/osade-agent-state.sh")
};
...
const QWEN_HOOK_INSTALL_NAME: &str = if cfg!(windows) {
    "osade-agent-session.ps1"
} else {
    "osade-agent-session.sh"
};
const QWEN_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/qwen/osade-agent-session.ps1")
} else {
    include_str!("assets/qwen/osade-agent-session.sh")
};
...
const CURSOR_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/cursor/osade-agent-state.ps1")
} else {
    include_str!("assets/cursor/osade-agent-state.sh")
};
const CURSOR_INTEGRATION_VERSION: u32 = 1;
#[cfg(windows)]
const ANTIGRAVITY_CLI_HOOK_INSTALL_NAME: &str = "osade-agent-state.ps1";
#[cfg(not(windows))]
const ANTIGRAVITY_CLI_HOOK_INSTALL_NAME: &str = "osade-agent-state.sh";
#[cfg(windows)]
const ANTIGRAVITY_CLI_HOOK_ASSET: &str =
    include_str!("assets/antigravity_cli/osade-agent-state.ps1");
#[cfg(not(windows))]
const ANTIGRAVITY_CLI_HOOK_ASSET: &str =
    include_str!("assets/antigravity_cli/osade-agent-state.sh");
const ANTIGRAVITY_CLI_INTEGRATION_VERSION: u32 = 3;
/// Antigravity CLI keys `hooks.json` by hook name, so every Osade entry lives
/// under one Osade-owned block that install rewrites and uninstall removes.
const ANTIGRAVITY_CLI_HOOK_BLOCK_NAME: &str = "osade";
...
const INTEGRATION_VERSION_MARKER: &str = "OSADE_INTEGRATION_VERSION=";
...
const MASTRACODE_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/mastracode/osade-agent-state.ps1")
} else {
    include_str!("assets/mastracode/osade-agent-state.sh")
};
...
const GROK_HOOK_CONFIG_INSTALL_NAME: &str = "osade.json";
const GROK_HOOK_ASSET: &str = if cfg!(windows) {
    include_str!("assets/grok/osade-agent-state.ps1")
} else {
    include_str!("assets/grok/osade-agent-state.sh")
};
