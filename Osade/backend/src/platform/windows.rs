...
const PANE_RUNTIME_MARKER_ENV_VAR: &str = "OSADE_PANE_RUNTIME_ID";
...
pub(crate) fn create_remote_ssh_config_dir(_control_socket_name: &str) -> std::io::Result<PathBuf> {
    ...
    Err(std::io::Error::new(
        ...
        "failed to create private osade ssh config directory",
    ))
}
...
pub fn show_desktop_notification(title: &str, body: Option<&str>) -> std::io::Result<bool> {
    ...
    std::thread::Builder::new()
        .name("osade-windows-notification".into())
...
fn show_desktop_notification_on_thread(
    ...
    let window_name = wide_null("Osade notifications");
    let hwnd = unsafe {
        CreateWindowExW(
            0,
            class_name.as_ptr(),
    ...
    if unsafe { Shell_NotifyIconW(NIM_MODIFY, &notification) } == 0 {
        ...
        let _ = ready_tx.send(Err(std::io::Error::other(
            "failed to show Osade desktop notification",
        )));
        return;
    }
...
//
// Windows IMEs live in the terminal-emulator process, not in osade. Empirically:
...
/// `SendMessageW` would block osade's client thread until that process responds
...
mod tests {
    ...
    fn windows_shells_round_trip_agent_arguments_through_a_real_command() {
        ...
        let run_command = |shell: &str, command: &str, capture: &std::path::Path| {
            ...
            process
                .env("PATH", &path)
                .env("OSADE_ARGV_CAPTURE", capture)
        ...
        fs::write(
            base.join("pi.ps1"),
            "Set-Content -LiteralPath $env:OSADE_ARGV_CAPTURE -Value @(\"$($args[0])\", \"$($args[1])\", \"$($args[2])\", \"$($args[3])\", \"$($args[4])\", \"$($args[5])\", \"$($args[6])\")\r\n",
        )
        .unwrap();
        for shell in ["powershell.exe", "cmd.exe"] {
    ...
    const CONSOLE_TEST_CHILD_ENV: &str = "OSADE_TEST_CONSOLE_CHILD_MODE";
    const CONSOLE_TEST_PARENT_PID_ENV: &str = "OSADE_TEST_CONSOLE_PARENT_PID";
    const WMI_DAEMON_TEST_CHILD_ENV: &str = "OSADE_TEST_WMI_DAEMON_CHILD";
    ...
    fn windows_process_cwd_reads_child_launch_directory() {
        let cwd = std::env::temp_dir().join(format!("osade-cwd-test-{}", std::process::id()));
    ...
    fn scrollback_editor_argv_uses_editor_env_and_appends_path() {
        let path = std::path::Path::new(r"C:\Users\User\AppData\Local\Temp\osade scrollback.txt");
    ...
    fn scrollback_editor_argv_falls_back_to_notepad() {
        let path = std::path::Path::new(r"C:\Temp\osade-scrollback.txt");
    ...
    fn process_environment_variable_parser_reads_case_insensitive_marker() {
        let environment: Vec<u16> = "PATH=C:\\Windows\0osade_pane_runtime_id=pane-a\0\0"
            .encode_utf16()
            .collect();
