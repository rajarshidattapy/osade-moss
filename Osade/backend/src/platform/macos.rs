...
mod tests {
    ...
    fn procargs2_env_reads_agent_hint_after_argv() {
        let buf = build_procargs2(
            ...
            &["nono", "run", "OSADE_AGENT=codex", "--", "claude"],
            &["PATH=/usr/bin", "OSADE_AGENT=claude", "TERM=xterm-256color"],
        );
    ...
    fn desktop_notification_falls_back_to_osascript_when_terminal_notifier_fails() {
        let path =
            std::env::temp_dir().join(format!("osade-osascript-args-{}", std::process::id()));
        let script = r#"
