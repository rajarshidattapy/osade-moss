...
const TERMINAL_SESSION_OBSERVE_USAGE: &str =
    "usage: osade terminal session observe <target> [--cols N] [--rows N]";
const TERMINAL_SESSION_CONTROL_USAGE: &str =
    "usage: osade terminal session control <target> [--takeover] [--cols N] [--rows N]";
...
fn channel_set(args: &[String]) -> std::io::Result<i32> {
    let Some(channel) = parse_channel_set_arg(args) else {
        eprintln!("usage: osade channel set <stable|preview>");
        return Ok(2);
    };
    ...
    println!(
        "Osade update channel set to {channel} in {}.",
        path.display()
    );
    ...
    if let Err(err) = crate::update::self_update(crate::update::SelfUpdateOptions::default()) {
        ...
        eprintln!("Run `osade update` to retry.");
        return Ok(1);
    }
...
fn print_channel_help() {
    eprintln!("osade channel commands:");
    eprintln!("  osade channel show                  print the configured update channel");
    eprintln!("  osade channel set <stable|preview>  choose the update channel");
}
...
fn config_check(args: &[String]) -> std::io::Result<i32> {
    match args {
        ...
        [flag] if matches!(flag.as_str(), "help" | "--help" | "-h") => {
            eprintln!("usage: osade config check");
            return Ok(0);
        }
        _ => {
            eprintln!("usage: osade config check");
...
fn config_reset_keys(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: osade config reset-keys");
        return Ok(2);
    }
    ...
    println!("Built-in v2 keybindings will apply after Osade restarts or reloads config.");
    println!("If a Osade server is running, run `osade server reload-config` to apply this now.");
    println!(
        "To restore: cp {} {}",
        backup_path.display(),
        path.display()
...
fn session_attach_help(args: &[String]) -> std::io::Result<i32> {
    if matches!(
        ...
        eprintln!("usage: osade session attach <name>");
        return Ok(0);
    }
    eprintln!("usage: osade session attach <name>");
    Ok(2)
...
fn session_list(args: &[String]) -> std::io::Result<i32> {
    let json = match parse_session_json_only(args, "usage: osade session list [--json]") {
        Ok(json) => json,
...
fn session_stop(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: osade session stop <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn session_delete(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: osade session delete <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn terminal_attach(args: &[String]) -> std::io::Result<i32> {
    let (terminal_id, takeover) = match parse_attach_target(
        args,
        "usage: osade terminal attach <terminal_id> [--takeover]",
    ) {
        Ok(parsed) => parsed,
        Err(code) => return Ok(code),
    };
...
fn terminal_title(args: &[String]) -> std::io::Result<i32> {
    match args.first().map(|arg| arg.as_str()) {
        Some("set") => {
            if args.len() != 2 {
                eprintln!("usage: osade terminal title set <title>");
                return Ok(2);
            }
        ...
        Some("clear") => {
            if args.len() != 1 {
                eprintln!("usage: osade terminal title clear");
                return Ok(2);
            }
            print_response(&send_request(&Request {
        ...
        Some("help" | "--help" | "-h") => {
            eprintln!("usage: osade terminal title set <title>");
            eprintln!("       osade terminal title clear");
            Ok(0)
        }
        _ => {
            eprintln!("usage: osade terminal title set <title>");
...
fn print_config_help() {
    eprintln!("osade config commands:");
    eprintln!("  osade config check  validate config.toml and print diagnostics");
    eprintln!("  osade config reset-keys  back up config.toml and remove custom keybindings");
}

fn print_terminal_help() {
    eprintln!("osade terminal commands:");
    eprintln!("  osade terminal attach <terminal_id> [--takeover]");
    eprintln!("  osade terminal session control <target> [--takeover] [--cols N] [--rows N]");
    eprintln!("  osade terminal session observe <target> [--cols N] [--rows N]");
    eprintln!("  osade terminal title set <title>");
    eprintln!("  osade terminal title clear");
    eprintln!("  detach from direct attach with ctrl+b q; send literal ctrl+b with ctrl+b ctrl+b");
}

fn print_session_help() {
    eprintln!("osade session commands:");
    eprintln!("  osade session list [--json]");
    eprintln!("  osade session attach <name>");
    eprintln!("  osade session stop <name> [--json]");
    eprintln!("  osade session delete <name> [--json]");
...
mod tests {
    ...
    fn parse_env_assignment_accepts_empty_values() {
        assert_eq!(
            super::parse_env_assignment("OSADE_ROLE=").unwrap(),
            ("OSADE_ROLE".to_string(), String::new())
        );
    }
    ...
    fn parse_env_assignment_requires_key_value_separator() {
        assert_eq!(
            super::parse_env_assignment("OSADE_ROLE").unwrap_err(),
