...
pub(crate) fn init_file_logging(file_name: &str) {
    ...
    let filter =
        EnvFilter::try_from_env("OSADE_LOG").unwrap_or_else(|_| EnvFilter::new("osade=info"));
...
pub(crate) fn help_log_paths_summary() -> String {
    ...
    format!(
        "{} (plus osade-client.log, osade-server.log)",
        dir.join("osade.log").display()
    )
}
...
mod tests {
    ...
    fn temp_log_path(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(unique).join("osade.log")
    }

    #[test]
    fn rotated_log_path_appends_numeric_suffix() {
        let path = PathBuf::from("/tmp/osade.log");
        assert_eq!(
            rotated_log_path(&path, 2),
            PathBuf::from("/tmp/osade.log.2")
        );
    }
    ...
    fn write_replaces_log_without_retained_files_when_size_limit_is_reached() {
        ...
        let writer = RotatingFileMakeWriter::new(dir.clone(), "osade.log", 8, 0).unwrap();
        {
