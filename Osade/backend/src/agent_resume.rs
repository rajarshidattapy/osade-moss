...
pub fn is_reserved_native_state_source(source: &str, agent: &str) -> bool {
    matches!(
        ...
        ("osade:claude", "claude")
            | ("osade:codex", "codex")
            | ("osade:copilot", "copilot")
            | ("osade:devin", "devin")
            | ("osade:droid", "droid")
            | ("osade:qodercli", "qodercli")
            | ("osade:qwen", "qwen")
            | ("osade:cursor", "cursor")
            | ("osade:grok", "grok")
    )
...
pub fn plan(source: &str, agent: &str, session_ref: &AgentSessionRef) -> Option<AgentResumePlan> {
    ...
    let argv = match (source, agent, session_ref.kind) {
        ("osade:claude", "claude", AgentSessionRefKind::Id) => {
            vec![
                "claude".into(),
                "--resume".into(),
        ...
        ("osade:codex", "codex", AgentSessionRefKind::Id) => {
            vec!["codex".into(), "resume".into(), session_ref.value.clone()]
        }
        ("osade:copilot", "copilot", AgentSessionRefKind::Id) => {
            vec!["copilot".into(), format!("--resume={}", session_ref.value)]
        }
        ("osade:devin", "devin", AgentSessionRefKind::Id) => {
            vec!["devin".into(), "--resume".into(), session_ref.value.clone()]
        }
        ("osade:droid", "droid", AgentSessionRefKind::Id) => {
            vec!["droid".into(), "--resume".into(), session_ref.value.clone()]
        }
        ("osade:kimi", "kimi", AgentSessionRefKind::Id) => {
            vec!["kimi".into(), "--session".into(), session_ref.value.clone()]
        }
        ("osade:mastracode", "mastracode", AgentSessionRefKind::Id) => {
            vec![
                "mastracode".into(),
                "--thread".into(),
                session_ref.value.clone(),
        ...
        ("osade:pi", "pi", AgentSessionRefKind::Path | AgentSessionRefKind::Id) => {
            vec!["pi".into(), "--session".into(), session_ref.value.clone()]
        }
        ("osade:omp", "omp", AgentSessionRefKind::Path | AgentSessionRefKind::Id) => {
        ...
        ("osade:hermes", "hermes", AgentSessionRefKind::Id) => {
            vec![
                "hermes".into(),
                "--resume".into(),
                session_ref.value.clone(),
        ...
        ("osade:opencode", "opencode", AgentSessionRefKind::Id) => {
            vec![
                "opencode".into(),
                "--session".into(),
                session_ref.value.clone(),
        ...
        ("osade:qodercli", "qodercli", AgentSessionRefKind::Id) => {
            vec![
                "qodercli".into(),
                "--resume".into(),
                session_ref.value.clone(),
        ...
        ("osade:qwen", "qwen", AgentSessionRefKind::Id) => {
            vec!["qwen".into(), "--resume".into(), session_ref.value.clone()]
        }
        ("osade:kilo", "kilo", AgentSessionRefKind::Id) => {
            vec!["kilo".into(), "--session".into(), session_ref.value.clone()]
        }
        ("osade:cursor", "cursor", AgentSessionRefKind::Id) => {
            vec![
                if cfg!(windows) {
                    "cursor-agent.cmd"
                } else {
        ...
        ("osade:antigravity_cli", "agy", AgentSessionRefKind::Id) => {
            vec![
                "agy".into(),
                "--conversation".into(),
                session_ref.value.clone(),
        ...
        ("osade:grok", "grok", AgentSessionRefKind::Id) => {
...
mod tests {
    ...
    fn native_state_reservation_excludes_full_lifecycle_sources() {
        assert!(is_reserved_native_state_source("osade:claude", "claude"));
        assert!(is_reserved_native_state_source("osade:codex", "codex"));
        assert!(is_reserved_native_state_source("osade:devin", "devin"));
        assert!(!is_reserved_native_state_source("osade:kimi", "kimi"));
    ...
    fn report_ref_prefers_pi_and_omp_paths_and_validates_values() {
        ...
        assert!(session_ref_from_report("osade:pi", "pi", Some("bad\nid".into()), None).is_none());
        assert!(
            session_ref_from_report("osade:pi", "pi", None, Some("relative.jsonl".into()))
                .is_none()
        );
        ...
        let session_ref =
            session_ref_from_report("osade:omp", "omp", Some("omp-id".into()), None).unwrap();
        ...
        assert_eq!(session_ref.value, "omp-id");
        assert!(
            session_ref_from_report("osade:omp", "omp", None, Some("relative.jsonl".into()))
                .is_none()
        );

        assert!(
            session_ref_from_report("osade:claude", "claude", None, Some(claude_session)).is_none()
        );

        let session_ref =
            session_ref_from_report("osade:copilot", "copilot", Some("copilot-id".into()), None)
                .unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "copilot-id");
        assert!(
            session_ref_from_report("osade:copilot", "copilot", None, Some(copilot_session))
                .is_none()
        );

        let session_ref =
            session_ref_from_report("osade:devin", "devin", Some("devin-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "devin-id");

        let session_ref =
            session_ref_from_report("osade:droid", "droid", Some("droid-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        ...
        let session_ref =
            session_ref_from_report("osade:kimi", "kimi", Some("kimi-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        ...
        let session_ref =
            session_ref_from_report("osade:kilo", "kilo", Some("kilo-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "kilo-id");

        let session_ref =
            session_ref_from_report("osade:qodercli", "qodercli", Some("qoder-id".into()), None)
                .unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "qoder-id");

        let session_ref =
            session_ref_from_report("osade:qwen", "qwen", Some("qwen-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "qwen-id");

        let session_ref =
            session_ref_from_report("osade:antigravity_cli", "agy", Some("agy-id".into()), None)
                .unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
    ...
    fn ids_are_data_not_shell_text() {
        let id = "abc; rm -rf /";
        let codex_plan = plan("osade:codex", "codex", &AgentSessionRef::id(id).unwrap()).unwrap();
        ...
        let devin_plan = plan("osade:devin", "devin", &AgentSessionRef::id(id).unwrap()).unwrap();
