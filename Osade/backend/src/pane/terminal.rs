...
use super::{
    input::{
        ...
        ghostty_prefers_osade_text_encoding,
    },
...
impl GhosttyPaneTerminal {
    ...
    fn encode_terminal_key_once(
        ...
        if ghostty_prefers_osade_text_encoding(&key) {
            return crate::input::encode_terminal_key(key, protocol);
        }
...
mod tests {
    ...
    fn process_pty_bytes_reports_latest_libghostty_pwd_callback() {
        ...
        let partial = pane.process_pty_bytes(pane_id, 0, b"\x1b]7;file:///tmp/osade%20", &tx);
        ...
        assert_eq!(
            completed.reported_cwd,
            Some(std::path::PathBuf::from("\\tmp\\osade repo"))
        );
    ...
    #[test]
    fn ghostty_char_keys_still_use_osade_encoding() {
        let (tx, _rx) = mpsc::channel(4);
        let mut terminal = crate::ghostty::Terminal::new(80, 24, 0).unwrap();
