...
pub fn run_server() -> io::Result<()> {
    ...
    let _api_server = match api::start_server_with_stop_control(
        ...
        Err(err) if err.kind() == io::ErrorKind::AddrInUse => {
            eprintln!("error: osade server is already running");
...
fn run_handoff_import_server(socket_path: &Path, token: &str) -> io::Result<()> {
    ...
    let result = rt.block_on(async {
        ...
        if std::env::var("OSADE_TEST_HANDOFF_IMPORT_FAIL").as_deref() == Ok("after_restored") {
...
fn print_ready_message(api_socket: &Path, client_socket: &Path) {
    eprintln!("osade server running; you can use any osade CLI command in another terminal.");
    ...
    eprintln!("did you mean to open the Osade TUI? run `osade`; you do not need `osade server`.");
}

/// Initialize logging for the server process.
fn init_logging() {
    crate::logging::init_file_logging("osade-server.log");
}
