...
//!
//! Osade is a terminal emulator, so `OSC 0`/`OSC 2` written by a pane stops at
//! Osade and never reaches the terminal Osade itself runs in. Without this the
...
impl App {
    ...
    /// Whether `ui.window_title` asks Osade to own the outer terminal title at
    /// all. When it does not, Osade leaves whatever the shell or `ssh` set.
