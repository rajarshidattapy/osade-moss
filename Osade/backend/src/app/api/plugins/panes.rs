...
impl App {
    ...
    fn plugin_pane_launch_env(
        ...
        env.push(("OSADE_ENV".to_string(), "1".to_string()));
        env.push(("OSADE_PLUGIN_ID".to_string(), plugin.plugin_id.clone()));
        env.push((
            "OSADE_PLUGIN_ENTRYPOINT_ID".to_string(),
            entrypoint.to_string(),
        ));
        env.push(("OSADE_PLUGIN_CONTEXT_JSON".to_string(), context_json));
...
fn plugin_pane_protected_env_key(key: &str) -> bool {
    matches!(
        ...
        crate::api::SOCKET_PATH_ENV_VAR
            ...
            | "OSADE_PLUGIN_ID"
            | "OSADE_PLUGIN_ROOT"
            | "OSADE_PLUGIN_CONFIG_DIR"
            | "OSADE_PLUGIN_STATE_DIR"
            | "OSADE_PLUGIN_ENTRYPOINT_ID"
            | "OSADE_PLUGIN_CONTEXT_JSON"
            | "OSADE_BIN_PATH"
    )
}
