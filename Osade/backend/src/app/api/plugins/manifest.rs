...
struct RawPluginManifest {
    ...
    version: String,
    #[serde(default)]
    min_osade_version: Option<String>,
    #[serde(default)]
...
pub(crate) fn load_plugin_manifest(
    ...
    let min_osade_version = validate_min_osade_version(raw.min_osade_version.as_deref())?;
    let description = raw
        .description
...
fn validate_min_osade_version(value: Option<&str>) -> Result<String, (&'static str, String)> {
    let Some(value) = value else {
        return Err((
            "invalid_plugin_min_osade_version",
            "plugin min_osade_version is required".to_string(),
        ));
    };
    let value = non_empty_trimmed(
        value,
        "invalid_plugin_min_osade_version",
        "plugin min_osade_version is required",
    )?;
    let required = crate::update::Version::parse(&value).ok_or_else(|| {
        (
            "invalid_plugin_min_osade_version",
            format!(
                "plugin min_osade_version must be a semantic version like {}",
    ...
    if required > current {
        return Err((
            ...
            format!("plugin requires Osade {required} or newer; current Osade is {current}"),
        ));
    }
