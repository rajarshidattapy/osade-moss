...
const DISABLE_SOUND_ENV: &str = "OSADE_DISABLE_SOUND";
#[cfg(any(windows, test))]
const WINDOWS_SOUND_PATH_ENV: &str = "OSADE_SOUND_PATH";
#[cfg(not(any(windows, target_os = "macos")))]
...
fn temp_sound_path() -> PathBuf {
    ...
    std::env::temp_dir().join(format!("osade-sound-{}-{id}.mp3", std::process::id()))
}
...
$Path = [Environment]::GetEnvironmentVariable('OSADE_SOUND_PATH', 'Process')
if ([string]::IsNullOrWhiteSpace($Path)) { throw 'OSADE_SOUND_PATH is not set' }
...
mod tests {
    ...
    fn windows_media_player_uses_process_environment_and_dispatcher() {
        ...
        assert!(script.contains("GetEnvironmentVariable('OSADE_SOUND_PATH', 'Process')"));
