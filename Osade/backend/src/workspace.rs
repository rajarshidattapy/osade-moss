...
pub struct Workspace {
    ...
    /// Explicit Osade-managed worktree grouping provenance.
...
mod tests {
    ...
    fn display_name_reads_cached_identity_without_rechecking_filesystem() {
        ...
        let root = std::env::temp_dir().join(format!(
            "osade-workspace-label-cache-{}-{stamp}",
            std::process::id()
        ));
    ...
    fn workspace_identity_follows_first_tab_root_pane_cwd() {
        ...
        terminals.insert(
            terminal_id.clone(),
            TerminalState::new(terminal_id, PathBuf::from("/osade-test/pion")),
        );
        ...
        assert_eq!(
            ...
            Some(PathBuf::from("/osade-test/pion"))
        );
    }
