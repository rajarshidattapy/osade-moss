...
use crate::config::ToastOsadePosition;
...
fn notification_show(args: &[String]) -> std::io::Result<i32> {
    let params = match parse_notification_show_args(args) {
        ...
        Err(NotificationShowArgError::Usage) => {
            eprintln!(
                "usage: osade notification show <title> [--body TEXT] [--position top-left|top-right|bottom-left|bottom-right] [--sound none|done|request]"
            );
            return Ok(2);
        }
...
fn parse_toast_position(value: &str) -> Result<ToastOsadePosition, NotificationShowArgError> {
    match value {
        "top-left" => Ok(ToastOsadePosition::TopLeft),
        "top-right" => Ok(ToastOsadePosition::TopRight),
        "bottom-left" => Ok(ToastOsadePosition::BottomLeft),
        "bottom-right" => Ok(ToastOsadePosition::BottomRight),
...
fn print_notification_help() {
    eprintln!("osade notification commands:");
    eprintln!(
        "  osade notification show <title> [--body TEXT] [--position top-left|top-right|bottom-left|bottom-right] [--sound none|done|request]"
    );
}
    ...
    fn notification_show_args_parse_title_body_and_position() {
        ...
        assert_eq!(
            ...
            NotificationShowParams {
                ...
                position: Some(ToastOsadePosition::TopRight),
