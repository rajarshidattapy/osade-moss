...
fn pane_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: osade pane get <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: osade pane get <pane_id>");
...
fn pane_current(args: &[String]) -> std::io::Result<i32> {
    let env_pane_id = std::env::var("OSADE_PANE_ID")
        .ok()
...
fn parse_pane_neighbor_args(args: &[String]) -> Result<PaneNeighborParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: osade pane neighbor --direction left|right|up|down [--pane ID|--current]"
                .into(),
        );
    };
...
fn parse_pane_focus_args(args: &[String]) -> Result<PaneFocusDirectionParams, String> {
    let params = parse_pane_neighbor_args(args).map_err(|_| {
        "usage: osade pane focus --direction left|right|up|down [--pane ID|--current]".to_string()
    })?;
...
fn parse_pane_resize_args(args: &[String]) -> Result<PaneResizeParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: osade pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
                .into(),
        );
    };
...
fn pane_rename(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: osade pane rename <pane_id> <label>|--clear");
        return Ok(2);
    };
    if args.len() < 2 {
        eprintln!("usage: osade pane rename <pane_id> <label>|--clear");
...
fn parse_pane_read_args(args: &[String]) -> Result<PaneReadParams, String> {
    const USAGE: &str = "usage: osade pane read <pane_id> [--source visible|recent|recent-unwrapped|detection] [--lines N] [--format text|ansi] [--ansi] [--raw]";
...
fn parse_pane_input_args(
    ...
    const USAGE: &str =
        "usage: osade pane input [<pane_id>|--pane ID|--current] --right-click osade|pane";
            ...
            "--current" => {
                ...
                pane_id = Some(
                    env_pane_id
                        ...
                        .ok_or("--current requires OSADE_PANE_ID")?,
                );
                index += 1;
            }
            "--right-click" => {
...
fn parse_right_click_target(value: &str) -> Result<PaneRightClickTarget, String> {
    match value {
        "osade" => Ok(PaneRightClickTarget::Osade),
...
fn parse_pane_split_args(
    ...
    let mut cwd = None;
    let mut focus = false;
    let mut right_click = PaneRightClickTarget::Osade;
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: osade pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--right-click osade|pane] [--focus] [--no-focus]"
                .into(),
        );
    };
...
fn pane_move_usage() -> String {
    "usage: osade pane move <pane_id> --tab <tab_id> --split right|down [--target-pane ID] [--ratio FLOAT] [--focus|--no-focus]\n       osade pane move <pane_id> --new-tab [--workspace ID] [--label TEXT] [--focus|--no-focus]\n       osade pane move <pane_id> --new-workspace [--label TEXT] [--tab-label TEXT] [--focus|--no-focus]"
        .into()
}
...
fn parse_pane_swap_args(args: &[String]) -> Result<PaneSwapParams, String> {
    ...
    match (directional, explicit) {
        ...
        _ => Err(
            "usage: osade pane swap --direction left|right|up|down [--pane ID|--current]\n       osade pane swap --source-pane ID --target-pane ID"
                .into(),
        ),
    }
}
...
fn pane_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: osade pane close <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: osade pane close <pane_id>");
...
fn pane_send_text(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: osade pane send-text <pane_id> <text>");
        return Ok(2);
    }
...
fn pane_send_keys(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: osade pane send-keys <pane_id> <key> [key ...]");
        return Ok(2);
    }
...
fn pane_run(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: osade pane run <pane_id> <command>");
        return Ok(2);
    }
...
fn parse_pane_wait_output_args(args: &[String]) -> Result<PaneWaitForOutputParams, String> {
    const USAGE: &str = "usage: osade pane wait-output <pane_id> (--match TEXT | --regex PATTERN) [--source visible|recent|recent-unwrapped] [--lines N] [--timeout MS] [--raw]";
...
fn pane_report_agent(args: &[String]) -> std::io::Result<i32> {
    const USAGE: &str = "usage: osade pane report-agent <pane_id> --source ID --agent LABEL --state idle|working|blocked|unknown [--message TEXT] [--seq N] [--agent-session-id ID] [--agent-session-path PATH]";
...
fn pane_report_agent_session(args: &[String]) -> std::io::Result<i32> {
    const USAGE: &str = "usage: osade pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH] [--session-start-source SOURCE]";
...
fn pane_release_agent(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: osade pane release-agent <pane_id> --source ID --agent LABEL [--seq N]");
        return Ok(2);
    };
...
fn pane_report_metadata(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: osade pane report-metadata <pane_id> --source ID [--agent LABEL] [--applies-to-source ID] [--title TEXT|--clear-title] [--display-agent TEXT|--clear-display-agent] [--state-label STATUS=TEXT] [--clear-state-labels] [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
        return Ok(2);
    };
...
fn print_pane_help() {
    eprintln!("osade pane commands:");
    eprintln!("  osade pane list [--workspace <workspace_id>]");
    eprintln!("  osade pane current [--pane ID|--current]");
    eprintln!("  osade pane get <pane_id>");
    eprintln!("  osade pane layout [--pane ID|--current]");
    eprintln!("  osade pane process-info [--pane ID|--current]");
    eprintln!("  osade pane neighbor --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  osade pane edges [--pane ID|--current]");
    eprintln!("  osade pane focus --direction left|right|up|down [--pane ID|--current]");
    eprintln!(
        "  osade pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
    );
    eprintln!("  osade pane zoom [<pane_id>|--pane ID|--current] [--toggle|--on|--off]");
    eprintln!("  osade pane rename <pane_id> <label>|--clear");
    eprintln!("  osade pane read <pane_id> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!("  osade pane input [<pane_id>|--pane ID|--current] --right-click osade|pane");
    eprintln!(
        "  osade pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--right-click osade|pane] [--focus] [--no-focus]"
    );
    eprintln!("  osade pane swap --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  osade pane swap --source-pane ID --target-pane ID");
    eprintln!("  osade pane move <pane_id> --tab <tab_id> --split right|down [--target-pane ID] [--ratio FLOAT] [--focus|--no-focus]");
    eprintln!("  osade pane move <pane_id> --new-tab [--workspace ID] [--label TEXT] [--focus|--no-focus]");
    eprintln!("  osade pane move <pane_id> --new-workspace [--label TEXT] [--tab-label TEXT] [--focus|--no-focus]");
    eprintln!("  osade pane close <pane_id>");
    eprintln!("  osade pane send-text <pane_id> <text>");
    eprintln!("  osade pane send-keys <pane_id> <key> [key ...]");
    eprintln!("  osade pane wait-output <pane_id> (--match TEXT | --regex PATTERN) [--source visible|recent|recent-unwrapped] [--lines N] [--timeout MS] [--raw]");
    eprintln!("  osade pane report-agent <pane_id> --source ID --agent LABEL --state idle|working|blocked|unknown [--message TEXT] [--seq N] [--agent-session-id ID] [--agent-session-path PATH]");
    eprintln!("  osade pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH]");
    eprintln!("  osade pane release-agent <pane_id> --source ID --agent LABEL [--seq N]");
    eprintln!("  osade pane report-metadata <pane_id> --source ID [--agent LABEL] [--applies-to-source ID] [--title TEXT|--clear-title] [--display-agent TEXT|--clear-display-agent] [--state-label STATUS=TEXT] [--clear-state-labels] [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
    eprintln!("  osade pane run <pane_id> <command>");
}
...
mod tests {
    ...
    fn parse_pane_split_args_accepts_ratio() {
        ...
        assert_eq!(params.right_click, PaneRightClickTarget::Osade);
    }
    ...
    fn parse_pane_swap_args_rejects_mixed_forms() {
        ...
        assert!(err.contains("usage: osade pane swap"));
    }
    ...
    fn parse_pane_move_args_rejects_target_pane_without_tab() {
        ...
        assert!(err.contains("usage: osade pane move"));
    }
