const API_SCHEMA_JSON: &str = include_str!("../../docs/next/api/osade-api.schema.json");
...
fn api_snapshot(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: osade api snapshot");
        return Ok(2);
    }
...
fn schema_summary_text() -> std::io::Result<String> {
    ...
    Ok(format!(
        "Osade API schema\nprotocol: {}\nschema_version: {}\nschemas: {}\n\nUse `osade api schema --json` to print the full schema.\nUse `osade api schema --output PATH` to write it to a file.\n",
        protocol,
        schema_version,
        schemas.join(", ")
    ))
...
fn print_api_help() {
    eprintln!("osade api commands:");
    eprintln!("  osade api snapshot");
    eprintln!("  osade api schema [--json | --output PATH]");
}

fn print_api_schema_help() {
    eprintln!("usage: osade api schema [--json | --output PATH]");
}
...
mod tests {
    ...
    fn schema_summary_text_stays_human_sized() {
        ...
        assert!(text.contains("Osade API schema"));
        assert!(text.contains("Use `osade api schema --json`"));
        assert!(text.len() < 400);
    }
}
