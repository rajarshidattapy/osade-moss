...
fn load_conpty() -> ConPtyFuncs {
    ...
    if std::env::var("OSADE_WINDOWS_CONPTY").as_deref() == Ok("system") {
        log::warn!("OSADE_WINDOWS_CONPTY=system disables Osade's bundled ConPTY");
        return system;
    }
    ...
    match app_local_conpty_path() {
        Ok(Some(path)) => load_app_local_conpty(&path).unwrap_or_else(|error| {
            panic!(
                "failed to load verified app-local ConPTY from {}: {error}; set OSADE_WINDOWS_CONPTY=system to use the Windows system ConPTY",
                path.display()
            )
        }),
        Ok(None) => system,
        Err(error) => panic!(
            "Osade's app-local ConPTY bundle is invalid: {}; reinstall Osade or set OSADE_WINDOWS_CONPTY=system to use the Windows system ConPTY",
            error
        ),
    }
}
...
fn app_local_conpty_path() -> Result<Option<PathBuf>, String> {
    ...
    let executable = std::env::current_exe()
        .map_err(|error| format!("cannot locate the Osade executable: {error}"))?;
    let executable_dir = executable
        .parent()
        .ok_or_else(|| "Osade executable has no parent directory".to_string())?;
    ...
    let mut expected = BTreeSet::from(["osade-conpty.json".to_string()]);
    ...
    let marker = bundle.join("osade-conpty.json");
