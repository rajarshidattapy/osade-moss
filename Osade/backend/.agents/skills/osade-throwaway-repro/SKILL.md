---
name: osade-throwaway-repro
description: Create and control a disposable named Osade session from inside an existing Osade session. Use for isolated Osade runtime, pane, terminal, process, API, persistence, or agent reproductions that should be driven through the CLI/API without touching the default session.
---

# Osade throwaway reproduction

Use a disposable named Osade session when a reproduction needs a real Osade server, panes, PTYs, agents, or socket API without risking the user's main session.

The temporary TUI only keeps the disposable session attached and supplies terminal geometry. Drive the reproduction from the parent session through Osade's CLI/API. Do not manually operate the nested TUI unless the bug specifically requires client input.

## Non-negotiable safety

- Never run the reproduction in the default session.
- Never stop, restart, delete, or kill the main Osade server.
...
Confirm the caller is inside Osade and inspect the relevant help before doing anything:
...
Inspect nested command help before using unfamiliar or potentially mutating commands. Do not run bare `osade` for discovery because it launches or attaches the TUI.

Record which Osade binary and version the reproduction tests. If testing a checkout build, follow the repository's instructions for running that build instead of silently substituting the installed binary.

## Create the outer pane

Create a sibling shell pane in the current tab without moving focus. Use an available Osade layout tool when the harness provides one. Otherwise use the installed pane split command after checking its help.
...
env \
  -u OSADE_SOCKET_PATH \
  -u OSADE_CLIENT_SOCKET_PATH \
  -u OSADE_RUNTIME_SESSION \
  -u OSADE_WORKSPACE_ID \
  -u OSADE_TAB_ID \
  -u OSADE_PANE_ID \
  osade --session <session-name>
```
...
env \
  ...
  -u OSADE_TAB_ID \
  -u OSADE_PANE_ID \
  OSADE_RUNTIME_SESSION=<session-name> \
  osade pane list
```
...
Named sessions isolate runtime state, sockets, panes, and persistence. They still share global Osade configuration and agent manifest overrides by default. Check configuration provenance when it could affect the reproduction. Do not modify shared configuration merely to make the test pass.
...
Use agent commands only after Osade recognizes a coding agent:
...
Before launching an agent, inspect its installed `--version` and `--help`. Pass native agent arguments after Osade's argument separator.
...
Read the named session directory and socket from `osade session list` instead of assuming their paths. Keep large evidence under `/var/tmp` unless the user asks to preserve it elsewhere.
