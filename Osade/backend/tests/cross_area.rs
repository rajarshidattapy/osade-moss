...
use support::{
    ...
    unregister_spawned_osade_pid, CURRENT_ENDPOINT_PROTOCOL_GENERATION as CURRENT_PROTOCOL,
...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/osade-cross-area-test-{}-{nanos}",
        std::process::id()
    ))
}
...
fn spawn_server(config_home: &Path, runtime_dir: &Path, api_socket_path: &Path) -> SpawnedOsade {
...
fn cross_area_detach_and_reattach_preserves_state() {
    ...
    // Use osade: create a workspace and write output into its pane.
...
fn cross_area_server_kill_then_restart_and_reconnect() {
    ...
    cleanup_spawned_osade(server2, base);
}
