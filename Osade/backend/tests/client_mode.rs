...
use support::{
    ...
    register_spawned_osade_pid, unregister_spawned_osade_pid, wait_for_client_shell_bootstrap,
...
fn direct_attach_initial_mouse_capture_follows_config() {
    ...
    drop(spawned_server);
    cleanup_spawned_osade(attach, base);
}
...
fn client_sees_headless_startup_config_diagnostic() {
    ...
    assert!(
        wait_until(Duration::from_secs(8), Duration::from_millis(20), || {
            ...
            output.contains("config.toml") && output.contains("osade config check")
        }),
    ...
    drop(spawned);
    cleanup_spawned_osade(client, base);
}
...
fn server_unreachable_shows_clear_error() {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_osade"))
        .arg("client")
        .env("OSADE_DISABLE_SOUND", "1")
    ...
    assert!(
        stderr.contains("Is osade server running?"),
        ...
        while Instant::now() < deadline {
            match thin_reader.read(&mut buf) {
                Ok(n) if n > 0 => {
                    ...
                    if output.to_lowercase().contains("osade:") {
                        break;
                    }
                }
...
) -> (SpawnedOsade, SpawnedOsade, SharedOutput) {
...
fn attach_thin_client_with_config(
    ...
    while Instant::now() < deadline {
        ...
        if out.to_lowercase().contains("osade:") {
            break;
        }
        thread::sleep(Duration::from_millis(30));
    }
...
fn client_shell_detaches_restores_and_freshly_reattaches_to_current_state() {
    ...
    drop(server);
    cleanup_spawned_osade(client_b, base);
}
...
fn assert_client_restores_terminal(trigger: impl FnOnce(&mut SpawnedOsade, &mut SpawnedOsade)) {
    let _lock = test_lock();
    let base = unique_test_dir();
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    ...
    // SpawnedOsade::Drop kills and reaps both processes with a bounded wait.
    drop(spawned_server);
    cleanup_spawned_osade(thin_client, base);
}
...
fn read_until_client_attaches(client: &SpawnedOsade) -> String {
...
fn pane_spawn_cwd_fallback_in_server() {
    ...
    drop(spawned);
    cleanup_spawned_osade(client_shell, base);
}
