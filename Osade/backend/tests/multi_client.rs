...
use support::{
    ...
    register_spawned_osade_pid, send_client_shell_focus, send_detach, unregister_spawned_osade_pid,
...
fn spawn_server(config: &Path, runtime: &Path, api: &Path) -> SpawnedOsade {
    fs::create_dir_all(config.join("osade")).unwrap();
    fs::create_dir_all(runtime).unwrap();
    register_runtime_dir(runtime);
    fs::write(config.join("osade/config.toml"), "onboarding = false\n").unwrap();
...
fn spawn_client(config: &Path, runtime: &Path, api: &Path) -> SpawnedOsade {
    register_runtime_dir(runtime);
    let pair = native_pty_system()
        .openpty(PtySize {
            rows: 24,
...
fn cleanup(server: SpawnedOsade, base: PathBuf) {
    drop(server);
...
fn same_tab_geometry_follows_meaningful_client_activity() {
    ...
    let api = runtime.join("osade.sock");
    let clients = runtime.join("osade-client.sock");
