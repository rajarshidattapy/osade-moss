...
pub(super) fn managed_github_plugin_dir(config_home: &Path) -> PathBuf {
    config_home.join("osade-dev").join("plugins").join("github")
}
...
pub(super) fn create_committed_repo(path: &Path) {
    ...
    run_git(path, &["config", "user.email", "osade@example.invalid"]);
    run_git(path, &["config", "user.name", "Osade Test"]);
    fs::write(path.join("README.md"), "test\n").unwrap();
    run_git(path, &["add", "README.md"]);
    run_git(path, &["commit", "--quiet", "-m", "initial"]);
}

pub(super) struct SpawnedOsade {
...
pub(super) fn cleanup_spawned_osade(spawned: SpawnedOsade, base: PathBuf) {
    drop(spawned);
...
pub(super) fn spawn_osade(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
) -> SpawnedOsade {
    spawn_osade_with_config(
        config_home,
        runtime_dir,
        socket_path,
        None,
...
pub(super) fn spawn_osade_with_pane_history(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
) -> SpawnedOsade {
...
pub(super) fn spawn_named_server(
    ...
    register_spawned_osade_pid(Some(child.id()));
...
pub(super) fn run_named_cli_with_env_and_socket_override(
    ...
    if let Some(socket_override) = socket_override {
        command.env("OSADE_SOCKET_PATH", socket_override);
    } else {
        command.env_remove("OSADE_SOCKET_PATH");
    }
...
pub(super) fn spawn_osade_with_path(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
...
pub(super) fn spawn_osade_with_config(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
    path_override: Option<&Path>,
...
pub(super) fn parse_cli_json_output(
    ...
    serde_json::from_slice(&output.stdout).unwrap_or_else(|err| {
        panic!(
            "failed to parse JSON response for `osade {}`: {}\nstdout: {}\nstderr: {}",
            args.join(" "),
            err,
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
