...
fn named_sessions_use_separate_servers_and_workspace_state() {
    ...
    assert!(alpha_session["socket_path"]
        .as_str()
        .unwrap()
        .ends_with("/sessions/alpha/osade.sock"));
...
fn dead_server_cli_reports_one_session_aware_json_line() {
    fn assert_server_not_running(
        ...
        assert_eq!(
            ...
            format!(
                "no osade server is running at {}; run `{attach_command}` to start or attach it",
                socket_path.display()
            )
        );
    }
    ...
    assert_server_not_running(missing, &named_socket, "osade session attach foo");

    let stale_socket = runtime_dir.join("stale.sock");
    drop(UnixListener::bind(&stale_socket).unwrap());
    let stale = Command::new(env!("CARGO_BIN_EXE_osade"))
        .args(["workspace", "create"])
        .env("XDG_CONFIG_HOME", &config_home)
        .env("XDG_RUNTIME_DIR", &runtime_dir)
        .env("OSADE_SOCKET_PATH", &stale_socket)
        .env("OSADE_RUNTIME_SESSION", "unrelated")
        .env_remove("OSADE_CLIENT_SOCKET_PATH")
        .env_remove("OSADE_ENV")
        .output()
        .unwrap();
    assert_server_not_running(stale, &stale_socket, "osade");
...
fn integration_commands_run_locally_when_server_is_missing() {
    ...
    let expected_extension = extensions_dir.join("osade-agent-state.ts");
    assert!(
        !expected_extension.exists(),
        "test setup should start without extension file"
    );

    let workspace_list = Command::new(env!("CARGO_BIN_EXE_osade"))
        .args(["workspace", "list"])
        .env("OSADE_SOCKET_PATH", &missing_socket)
        .env("HOME", &home_dir)
        .output()
        .unwrap();
    assert_eq!(workspace_list.status.code(), Some(1));

    let integration_install = Command::new(env!("CARGO_BIN_EXE_osade"))
    ...
    let integration_status = Command::new(env!("CARGO_BIN_EXE_osade"))
    ...
    let integration_uninstall = Command::new(env!("CARGO_BIN_EXE_osade"))
...
fn integration_status_outdated_only_prints_action_for_legacy_install() {
    ...
    fs::create_dir_all(&extensions_dir).unwrap();
    fs::write(
        extensions_dir.join("osade-agent-state.ts"),
        "// legacy osade integration\n",
    )
    .unwrap();
    ...
    assert!(stderr.contains("installed osade integrations need updating"));
    assert!(stderr.contains("osade integration install pi"));
...
fn server_stop_command_shuts_down_running_server() {
    ...
    let mut osade = spawn_osade(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    ...
    let pid = osade.child.process_id();
    let exit_status = osade.child.wait().unwrap();
...
fn server_stop_then_restart_restores_pane_history() {
    ...
    let mut osade = spawn_osade_with_pane_history(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    wait_for_socket(&client_socket, Duration::from_secs(5));
    ...
    let restarted = spawn_osade_with_pane_history(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    wait_for_socket(&client_socket, Duration::from_secs(5));
    ...
    cleanup_spawned_osade(restarted, base);
}
