...
fn help_commands_exit_successfully() {
    ...
    for args in help_cases {
        ...
        assert!(
            ...
            "osade {} failed: status={:?} stdout={} stderr={}",
            args.join(" "),
            output.status.code(),
...
fn root_and_command_group_help_point_agents_to_plain_text_docs() {
    for args in [&["--help"][..], &["agent", "--help"][..]] {
        let output = Command::new(env!("CARGO_BIN_EXE_osade"))
            .args(args)
            .env_remove("OSADE_SOCKET_PATH")
            .env_remove("OSADE_CLIENT_SOCKET_PATH")
            .env_remove("OSADE_ENV")
            .output()
            .unwrap();
        assert!(output.status.success(), "osade {} failed", args.join(" "));
        ...
        for expected in [
            ...
            assert!(
                ...
                "osade {} help did not contain {expected:?}: {stdout}",
                args.join(" ")
            );
        }
    }
...
fn removed_wait_and_agent_send_commands_are_rejected() {
    let wait = Command::new(env!("CARGO_BIN_EXE_osade"))
    ...
    let help = Command::new(env!("CARGO_BIN_EXE_osade"))
        .arg("--help")
        .output()
        .unwrap();
    assert!(!String::from_utf8_lossy(&help.stdout).contains("osade wait <subcommand>"));

    let send = Command::new(env!("CARGO_BIN_EXE_osade"))
        .args(["agent", "send", "reviewer", "hello"])
        .output()
        .unwrap();
    assert_eq!(send.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&send.stderr);
    assert!(stderr.contains("osade agent send-keys"));
    assert!(!stderr.contains("osade agent send <"));
}
...
fn agent_cli_rejects_invalid_wait_and_rename_grammar_locally() {
    for args in [
        ...
        let output = Command::new(env!("CARGO_BIN_EXE_osade"))
            .args(args)
            .env("OSADE_SOCKET_PATH", "/nonexistent/osade.sock")
            .output()
            .unwrap();
        assert_eq!(
            output.status.code(),
...
fn completion_command_prints_zsh_script_without_session_startup() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("#compdef osade"), "stdout: {stdout}");
    assert!(
        !stdout.contains("--cwd=[]"),
...
fn root_help_advertises_api_schema_command_group() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("osade api <subcommand>"),
...
fn api_schema_default_output_is_a_short_summary() {
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Osade API schema"), "stdout: {stdout}");
    assert!(
        stdout.contains("Use `osade api schema --json`"),
        "stdout: {stdout}"
    );
    assert!(
        stdout.len() < 400,
...
fn api_schema_output_writes_bundled_schema_to_file() {
    let base = unique_test_dir();
    fs::create_dir_all(&base).unwrap();
    let schema_path = base.join("osade-api.schema.json");
