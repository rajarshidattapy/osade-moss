...
fn plugin_link_works_offline_and_is_global() {
    ...
    fs::write(
        plugin_dir.join("osade-plugin.toml"),
        r#"
id = "example.offline"
name = "Offline Plugin"
version = "0.1.0"
...
"#,
    ...
    .unwrap();
    run_git(&source_repo, &["add", "global-plugin/osade-plugin.toml"]);
...
version = "0.1.0"
min_osade_version = "0.6.10"
description = "Apply a preferred Osade layout"
...
[[build]]
command = ["sh", "-c", "echo built > built.txt; if [ -n \"$OSADE_RUNTIME_SESSION\" ]; then echo \"$OSADE_RUNTIME_SESSION\" > leaked-session.txt; fi"]
...
"#,
    ...
    run_git(
        &source_repo,
        &["add", "worktree-bootstrap/osade-plugin.toml"],
    );
    run_git(&source_repo, &["commit", "--quiet", "-m", "add plugin"]);
    ...
    let install = run_named_cli_with_env(
        ...
        &[
            ("GIT_CONFIG_GLOBAL", &git_config),
            ("OSADE_RUNTIME_SESSION", Path::new("leaked-session")),
        ],
    );
    assert!(
        install.status.success(),
    ...
    assert_eq!(plugin["source"]["repo"], "osade-plugin-examples");
    ...
    assert!(
        !managed_path
            ...
            .exists(),
        "build command should not inherit OSADE_RUNTIME_SESSION"
    );
...
"#,
    ...
    .unwrap();
    run_git(&source_repo, &["add", "build-fail/osade-plugin.toml"]);
    run_git(
        &source_repo,
...
[[build]]
command = ["definitely-missing-osade-build-tool-xyz"]
...
"#,
    ...
    .unwrap();
    run_git(&source_repo, &["add", "missing-tool/osade-plugin.toml"]);
    run_git(
        &source_repo,
    ...
    assert!(stderr.contains("  build: 1/1"), "{stderr}");
    assert!(
        stderr.contains("  command: definitely-missing-osade-build-tool-xyz"),
        "{stderr}"
    );
...
"#,
    ...
    fs::write(
        ...
        r#"cat > osade-plugin.toml <<'EOF'
id = "example.manifest-mutator"
name = "Manifest Mutator"
version = "0.1.0"
min_osade_version = "0.0.1"
platforms = ["linux", "macos", "windows"]
...
"#,
    ...
    let stderr = String::from_utf8_lossy(&install.stderr);
    assert!(
        stderr.contains("plugin build changed osade-plugin.toml after install preview"),
        "{stderr}"
    );
...
fn plugin_install_restores_previous_checkout_when_registration_fails() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let socket_path = runtime_dir.join("fake-osade.sock");
    let source_repo = base.join("source-repo");
    let plugin_dir = source_repo.join("worktree-bootstrap");
    fs::create_dir_all(&plugin_dir).unwrap();
    create_committed_repo(&source_repo);
            ...
            serde_json::json!({
                ...
                "result": {
                    ...
                    "plugins": [{
                        ...
                        "version": "0.1.0",
                        "min_osade_version": "0.6.10",
                        "manifest_path": managed_checkout_for_server.join("osade-plugin.toml").display().to_string(),
                        "plugin_root": managed_checkout_for_server.display().to_string(),
                        "enabled": true,
                        "source": {
                            "kind": "github",
                            "owner": "ogulcancelik",
                            "repo": "osade-plugin-examples",
