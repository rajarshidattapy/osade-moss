...
use support::{
    cleanup_test_base, register_runtime_dir, register_spawned_osade_pid,
    unregister_spawned_osade_pid,
};
...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/osade-autodetect-test-{}-{nanos}",
        std::process::id()
    ))
}
...
/// Spawn `osade` (no subcommand) — the auto-detect launch path.
fn spawn_osade_auto(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
...
fn run_cli(socket_path: &Path, args: &[&str]) -> std::process::Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_osade"));
    command.args(args);
    command.env("OSADE_SOCKET_PATH", socket_path);
...
/// Running `osade` with no server present starts a server
...
fn auto_detect_no_server_spawns_server_and_attaches() {
    ...
    // Run `osade` (no subcommand) — should auto-detect, spawn server, attach as client.
    let osade = spawn_osade_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    let client_pid = osade.child.process_id().expect("client should have PID");
    assert!(
...
/// Running `osade` with a server already running attaches
...
fn auto_detect_server_running_attaches_directly() {
    ...
    // Run `osade` (no subcommand) — should detect the running server and attach.
    let client = spawn_osade_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    cleanup_spawned_osade(client, PathBuf::from("/nonexistent"));
    cleanup_spawned_osade(server, base);
}

/// Socket path resolution is consistent between server and client.
/// Both derive the client socket from the `OSADE_SOCKET_PATH` override,
...
fn auto_detect_socket_path_consistency() {
    ...
    // Run `osade` with custom socket paths.
    let osade = spawn_osade_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
...
fn cli_subcommands_work_through_server() {
    ...
    // Test `osade workspace list` through the server's API socket.
    ...
    // Test `osade pane list` through the server's API socket.
...
/// persists after the client exits, and a new `osade` can reattach.
#[test]
fn auto_detect_server_persists_and_reattaches() {
    ...
    // Run `osade` — auto-detect spawns server + attaches client.
    let mut client1 = spawn_osade_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    // Run `osade` again — should detect the running server and reattach.
    let client2 = spawn_osade_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    thread::sleep(Duration::from_millis(500));
    ...
    cleanup_spawned_osade(client2, PathBuf::from("/nonexistent"));
    cleanup_test_base(&base);
}
...
fn auto_detect_default_socket_path_from_config_dir() {
    ...
    // Don't set OSADE_SOCKET_PATH or OSADE_CLIENT_SOCKET_PATH.
    ...
    let api_socket = config_home.join(app_dir_name).join("osade.sock");
    let client_socket = config_home.join(app_dir_name).join("osade-client.sock");
    ...
    cmd.env_remove("OSADE_SOCKET_PATH");
    cmd.env_remove("OSADE_CLIENT_SOCKET_PATH");

    let child = pair.slave.spawn_command(cmd).unwrap();
    register_spawned_osade_pid(child.process_id());
    drop(pair.slave);
    let server = SpawnedOsade {
        _master: pair.master,
        child,
    };
...
fn auto_detect_writes_client_and_server_logs_to_separate_files() {
    ...
    let spawned = spawn_osade_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    let client_log = log_dir.join("osade-client.log");
    let server_log = log_dir.join("osade-server.log");
    let monolith_log = log_dir.join("osade.log");
    ...
    assert!(
        ...
        "persistent client logs should not land in osade.log: {monolith_content}"
    );
...
fn auto_detect_respects_nested_guard_before_auto_attach() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_osade"))
        .env("XDG_CONFIG_HOME", &config_home)
        .env("XDG_RUNTIME_DIR", &runtime_dir)
        .env("OSADE_SOCKET_PATH", &api_socket)
        .env_remove("OSADE_CLIENT_SOCKET_PATH")
        .env("OSADE_ENV", "1")
        .output()
        .unwrap();
    ...
    assert!(
        stderr.contains("nested osade is disabled by default"),
