...
baseline=${OSADE_PERF_BASELINE_BIN:-}
...
root=$(mktemp -d /var/tmp/osade-release-perf-smoke.XXXXXX)
cleanup() { rm -rf "$root"; }
trap cleanup EXIT
trap 'exit 130' INT
trap 'exit 143' TERM
...
seconds=${OSADE_PERF_SAMPLE_SECONDS:-10}
warmup=${OSADE_PERF_WARMUP_SECONDS:-3}
