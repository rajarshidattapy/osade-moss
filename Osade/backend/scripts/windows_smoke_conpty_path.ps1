...
$fakeDir = Join-Path ([System.IO.Path]::GetTempPath()) "osade-fake-conpty-$([guid]::NewGuid().ToString('N'))"
...
$oldPath = $env:PATH
$oldSession = $env:OSADE_RUNTIME_SESSION
$oldSocket = $env:OSADE_SOCKET_PATH
$oldClientSocket = $env:OSADE_CLIENT_SOCKET_PATH
$env:PATH = "$fakeDir;$oldPath"
$env:OSADE_RUNTIME_SESSION = $Session
Remove-Item Env:OSADE_SOCKET_PATH, Env:OSADE_CLIENT_SOCKET_PATH -ErrorAction SilentlyContinue
...
try {
    ...
    if ($null -eq $oldSession) {
        Remove-Item Env:OSADE_RUNTIME_SESSION -ErrorAction SilentlyContinue
    } else {
        $env:OSADE_RUNTIME_SESSION = $oldSession
    }
    if ($null -eq $oldSocket) {
        Remove-Item Env:OSADE_SOCKET_PATH -ErrorAction SilentlyContinue
    } else {
        $env:OSADE_SOCKET_PATH = $oldSocket
    }
    if ($null -eq $oldClientSocket) {
        Remove-Item Env:OSADE_CLIENT_SOCKET_PATH -ErrorAction SilentlyContinue
    } else {
        $env:OSADE_CLIENT_SOCKET_PATH = $oldClientSocket
    }
