...
OSADE_BIN="${OSADE_BIN:-$ROOT/target/debug/osade}"
BASE="${BASE:-$(mktemp -d /tmp/osade-handoff-smoke.XXXXXX)}"
...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_osade "$session" server stop >/dev/null 2>&1 || true
  done
...
run_osade() {
  ...
  env -u OSADE_SOCKET_PATH \
    -u OSADE_CLIENT_SOCKET_PATH \
    -u OSADE_RUNTIME_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$OSADE_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/osade-dev' "$CONFIG_HOME"
  else
    printf '%s/osade-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/osade.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/osade-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/osade-dev/osade.sock | "$CONFIG_HOME"/osade-dev/sessions/*/osade.sock)
      ;;
    *)
...
echo "using osade: $OSADE_BIN"
echo "smoke base: $BASE"

cargo build --locked --manifest-path "$ROOT/Cargo.toml" >/dev/null
mkdir -p "$CONFIG_HOME/osade-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/osade-dev/config.toml"

for session in "${sessions[@]}"; do
  echo "starting smoke session $session at $(api_socket "$session")"
  run_osade "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
