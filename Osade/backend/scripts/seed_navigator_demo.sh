...
Seeds a running osade server with navigator demo workspaces, tabs, panes,
and fake agent states for recording the session navigator.

Environment:
  OSADE_NAV_SOCKET_PATH  API socket to target. Defaults to $HOME/.config/osade-dev/osade.sock.
  OSADE_NAV_CWD          Workspace cwd for created panes. Defaults to the repo root.
  OSADE_NAV_BIN          Osade binary to call. Defaults to cargo run from the repo.
USAGE
}
...
workspace_cwd="${OSADE_NAV_CWD:-$repo_dir}"
config_home="${XDG_CONFIG_HOME:-$HOME/.config}"
dev_socket="$config_home/osade-dev/osade.sock"
main_socket="$config_home/osade/osade.sock"
export OSADE_SOCKET_PATH="${OSADE_NAV_SOCKET_PATH:-$dev_socket}"

if [[ "$allow_main" != 1 && "$OSADE_SOCKET_PATH" == "$main_socket" ]]; then
  echo "refusing to seed main osade session: $OSADE_SOCKET_PATH" >&2
  echo "use OSADE_NAV_SOCKET_PATH for a dev socket, or pass --allow-main intentionally" >&2
  exit 1
fi

if [[ ! -S "$OSADE_SOCKET_PATH" ]]; then
  echo "osade socket not found: $OSADE_SOCKET_PATH" >&2
  echo "start a dev server first, or set OSADE_NAV_SOCKET_PATH" >&2
  exit 1
fi

cd "$repo_dir"

if [[ -n "${OSADE_NAV_BIN:-}" ]]; then
  run() { "$OSADE_NAV_BIN" "$@"; }
else
  run() { cargo run --quiet -- "$@"; }
fi
...
cat <<EOF
Seeded navigator demo data via $OSADE_SOCKET_PATH
