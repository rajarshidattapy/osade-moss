...
MARKER_PATH = PurePosixPath("conpty/osade-conpty.json")
...
def stage_bundle(
    ...
    if not osade_exe.is_file():
        raise ValueError(f"Osade executable does not exist: {osade_exe}")
    ...
    with zipfile.ZipFile(package_path) as archive, tempfile.TemporaryDirectory(
        prefix="osade-conpty-stage-", dir=output_dir.parent
    ) as temporary:
        validate_nuspec(archive, metadata["package"])
        staging = Path(temporary) / "bundle"
        staging.mkdir()
        shutil.copy2(osade_exe, staging / "osade.exe")
...
def expected_stage_files(metadata: dict[str, Any], architecture: str) -> set[str]:
    files = {"osade.exe", MARKER_PATH.as_posix()}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Package Osade with Microsoft's pinned ConPTY runtime")
    ...
    stage.add_argument("--osade-exe", type=Path, required=True)
